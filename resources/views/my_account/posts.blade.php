...
<table id="table" class="table table-striped nowrap" style="width: 90%;margin-left: 5%;margin-top: 25px;">
  <thead>
     <tr>
    ...
    @foreach ($posts as $post)
        <tr>
            ...
            <td>{{ $post->status }}</td>
            <td>
                <a href="{{route('post.edit_my_post', [$post->id])}}" class="btn btn-info btn-sm">@lang('Edit')  </a>
                <form method="POST" class="d-inline" onsubmit="return confirm('Move post to trash ?')" action="{{route('post.trash_my_post', $post->id)}}">
                    @csrf

...
<form role="form" class="form-horizontal" enctype="multipart/form-data" action="{{ route('patch_custom_script') }}" method="post" autocomplete="off" >
   ...
   <p class="text-muted">
      <span class="required">*</span> @lang('Please note that if you want to add a css/js code you have to include the css/js tag as well')
   </p>

    <div class="form-group">
       <label>@lang('Header Script')</label>
       <small class="text-muted">@lang('Codes inserted below will be injected between the header tag, during run time')</small>
     ...
     <div class="form-group">
       <label>@lang('Footer Script')</label>
       <small class="text-muted">@lang('Codes inserted below will be injected in the footer section, during run time')</small>

...
<hr>
<h4>@lang('Conversations')</h4>
<p class="text-muted"><small>@lang('Sorted based on most recent comment')</small></p>
...
<div id="thread_{{ $comment->id }}" class="{{ ($comment->user_id == auth()->user()->id) ? 'thread-member' : 'thread-customer' }} comment-box">
   ...
   <div class="comment-box-footer">
       @lang('Posted') : {{ convertToLocalTime($comment->created_at, 'd-M-Y h:i:s a') }}
   </div>
</div>
@endforeach
{{ $comments->links() }}

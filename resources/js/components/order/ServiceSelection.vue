<template>
  <div class="row">
    ...
    <h6>{{ $t("TYPE OF WORK AND DEADLINE") }}</h6>
    <div></div>
    <!-- <div class="seclector" >
                    <div style="font-weight: bold;">Newest First</div>
    ...
    <div class="form-group col-md-12">
      <label>{{ $t("Services") }}<span class="required">*</span></label>
      <details class="dropdown" id="dropdown">
        <summary role="button">
          <a class="button">
            {{ form.service_categories_model.name[locale] }}
          </a>
          <i class="fas fa-caret-down"></i>
        </summary>
        <ul>
          <li
            @click="setServices3(item, item.id, item.worklevel)"
            @change="setServices"
            v-for="(item, index) in filteredServices_categories"
            :key="index"
          >
            <a href="#" :class="[active_services == item.id ? 'active' : '']">
              {{
    ...
    <div class="form-group col-md-12">
      <label>{{ $t("ServiceType") }}<span class="required">*</span></label>
      <details
        class="dropdown"
        id="dropdown"
      ...
      >
        <summary role="button">
          ...
          <a class="button">{{
            chosenServType == true
              ? form.service_model.name[locale]
              : filteredServices[0].name[locale]
          }}</a>
          <i class="fas fa-caret-down"></i>
        </summary>
        <ul>
          <li
            @click="
              getAdditionalServices(form.service_model);
          ...
          >
            <a
              href="#"
              :class="[form.service_model.id == item.id ? 'active' : '']"
            >
              {{
                item.name[locale] == null ? item.name["en"] : item.name[locale]
              }}
    ...
    <div class="form-group col-md-12" v-if="show_worklevel">
      <label>{{ $t("Work Level") }}<span class="required">*</span></label>
         <details class="dropdown" id="dropdown">
        <summary role="button">
          <a class="button">
            {{ form.work_level_model.name[locale] }}
          </a>
          <i class="fas fa-caret-down"></i>
        </summary>
        <ul>
          <li
            @click="workLevelChanged(workLevel.id, index)"
            v-for="(workLevel, index) in filteredlevels"
            :key="index"
          >
            <a href="#" :class="[form.work_level_id === Number(workLevel.id) ? 'active' : '']">
             {{
              workLevel.name[locale] == null
                ? workLevel.name["en"]
                : workLevel.name[locale]
            }}
            </a>
          </li>
        </ul>
    ...
    >
      <div class="form-group col-md-6">
        <label>{{ $t("Number of pages") }}<span class="required">*</span></label>
        ...
        <div class="invalid-feedback d-block" v-if="errors.number_of_pages">
          {{ errors.number_of_pages[0] }}
        </div>
      </div>
      <div class="form-group col-md-6">
        <label>
          {{ $t("Spacing") }}<span class="required">*</span>
          <span
            data-toggle="tooltip"
    ...
    <div class="form-row">
      <div
        ...
        v-if="
          ...
          typeof params.words == 'string'
        "
      >
        <label>{{ $t("Number of Words") }}<span class="required">*</span></label>
        <div class="input-group mb-3">
          <div class="input-group-prepend">
            <button
              type="button"
        ...
        <div class="invalid-feedback d-block" v-if="errors.number_of_words">
      ...
      <div
        class="form-group"
        v-bind:class="{
          'col-md-6': form.service_model.price_type_id == pricingTypes.perWord,
          'col-md-12': form.service_model.price_type_id != pricingTypes.perWord,
        }"
      >
        <label>{{ $t("Urgency") }}<span class="required">*</span></label>
        <multiselect
          track-by="id"
          label="name"
          v-model="form.urgency_model"
          :options="filteredurgency"
        ></multiselect>
      </div>
    </div>
    ...
    <div v-if="additional_services.length > 0">
      ...
      <div
        class="card mb-3"
        v-for="addi_serv in additional_services"
        v-bind:key="addi_serv.id"
      >
          ...
          <div class="col-md-4">
            <div class="d-flex justify-content-center" style="margin-top: 40px">
              <a
                href="#"
                v-on:click.prevent="
                  additionalServiceChanged(addi_serv.id, addi_serv)
                "
              >
                <div
                  class="btn btn-block"
                  v-bind:class="getServiceContainerClass(addi_serv.id)"
                >
    ...
    <div v-if="user_id">
      <button
        :disabled="hasError"
        type="button"
        class="btn btn-Create btn-lg btn-block"
    ...
    <div v-else>
      ...
      <a
        :href="quest_order_url"
        class="btn btn-Quest btn-lg btn-block"
      >
  ...
  computed: {
    ...
    filteredServices() {
      if (this.params_service != null) {
        ...
        this.params_service_catg != null &&
        this.params_service == null
      ) {
        ...
        return this.services.filter((el) => {
          return (
            el.service_category_id == this.form.service_categories_model.id
          );
        });
      }
    },

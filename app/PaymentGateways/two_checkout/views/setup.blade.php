<form role="form" class="form-horizontal" enctype="multipart/form-data" action="{{ route('patch_settings_two_checkout') }}" method="post" autocomplete="off" >
   {{ csrf_field()  }}
   {{ method_field('PATCH') }}
   ...
   <div class="form-group">
      <label>Merchant Code / Account Number<span class="required">*</span></label>
       <input type="text" class="form-control {{ showErrorClass($errors,'merchant_code') }}" name="merchant_code" value="{{ old('merchant_code', optional($rec->keys)->merchant_code) }}">
       <div class="invalid-feedback">{{ showError($errors,'merchant_code') }}</div>
    </div>
    <div class="form-group">
